      ...
      <div className="relative overflow-hidden [mask-composite:intersect] [mask-image:linear-gradient(to_right,transparent,black_5rem),linear-gradient(to_left,transparent,black_5rem),linear-gradient(to_bottom,transparent,black_5rem),linear-gradient(to_top,transparent,black_5rem)]">
        <div className="grid h-[250px] w-[300px] gap-5 animate-skew-scroll grid-cols-1">
          {items.map((item) => (
            <div
              key={item.id}
              className="group flex items-center gap-2 cursor-pointer rounded-md border border-white/20 bg-gradient-to-b from-white/10 to-white/5 p-4 shadow-md transition-all duration-300 ease-in-out hover:scale-105 hover:-translate-x-1 hover:-translate-y-1 hover:shadow-xl backdrop-blur-sm"
            >
              <CheckCircleIcon className="h-6 w-6 mr-2 stroke-white/60 transition-colors group-hover:stroke-white" />
              <p className="text-white/80 transition-colors group-hover:text-white">
                {item.text}
              </p>
            </div>
          ))}
...
const defaultItems: TiltedScrollItem[] = [
  { id: "1", text: "Gemini AI Integration" },
